...
class OutlookCalendarConnector(BaseConnector):
    ...
    def has_updates(self, since: Optional[datetime] = None) -> bool:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.warning(f"Error checking for Outlook Calendar updates: {response.text}")
                return False
            ...
            logger.warning(f"Error checking for Outlook Calendar updates: {e}")
            return False
        ...
        try:
            ...
            for event in events:
                try:
                    ...
                    logger.warning(f"Error processing Outlook Calendar event {event.get('id')}: {e}")
                    continue

            return connector_data

        except Exception as e:
            logger.error(f"Error fetching Outlook Calendar updates: {e}")
            return []
    ...
    def _refresh_token(self, token_data: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(f"Outlook Calendar token refresh failed: {response.text}")
                return None
            ...
            logger.error(f"Error refreshing Outlook Calendar token: {e}")
            return None
    ...
    def _fetch_events(self, access_token: str, since: Optional[datetime], limit: int) -> List[Dict]:
        ...
        while url and len(events) < limit:
            ...
            if response.status_code != 200:
                logger.error(f"Error fetching Outlook Calendar events: {response.text}")
                break
...
"""
    ...
    def _load_oauth_app(self) -> dict:
        """Load user-stored OAuth app credentials (own config or outlook_mail fallback)."""
        f = self.config_dir / 'oauth_app.json'
        if f.exists():
            with open(f) as fh:
                return json.load(fh)
        # Fall back to outlook_mail credentials (same Azure app)
        parent = self.config_dir.parent / 'outlook_mail' / 'oauth_app.json'
        if parent.exists():
