#!/usr/bin/env python3
"""
Agentic Ingestion Pipeline V3 - OpenCode-Inspired Improvements

Key enhancements:
1. Fuzzy matching for section/file names
2. Validation feedback loops
3. Retry mechanism (max 3 attempts)
4. Anthropic-optimized prompts
    ...
    def validate(file_path: Path) -> List[str]:
        ...
        if citations_found:
            ...
            if unique_citations != expected:
                errors.append(
                    f"Citations not sequential: found {unique_citations}, "
                    f"expected {expected}"
                )
...
class AgenticIngestionPipelineV3:
    """
    Enhanced ingestion pipeline with fuzzy matching and validation.
    ...
    def __init__(self, vault_path: Path):
        ...
        print(f"🤖 Initialized agentic ingestion pipeline V3")
        print(f"📂 Vault: {self.vault_path}")
        print(f"🧠 Model: claude-haiku-4-5-20251001")
        print(f"✨ Features: fuzzy matching, validation loops, retry\n")
...
def main():
    """CLI interface for agentic ingestion V3."""
    if len(sys.argv) < 3:
        print("Usage: python agentic_ingest_v3.py <vault_path> <context> [source_json]")
        print("\nExample:")
        print('  python agentic_ingest_v3.py ~/my-vault "Got offer from Meta"')
        print('  python agentic_ingest_v3.py ~/my-vault "Interview" \'{"platform": "Gmail", ...}\'')
        sys.exit(1)
